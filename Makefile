...
$(static)/%.stamp: %
	mkdir -p $(@D)
	./scripts/ln-unique.sh $< $(@D)
	touch $@

%_asmap.m4: %_asmap.sed
	printf '%s\n' \
	       $(addsuffix -*,$(basename $(basename $(filter-out $<,$^)))) | \
	sed s,$(static),, | sort | uniq | ./scripts/map-asset.sh $$(cat $<) >$@
...
hot-proxy:
	$(caddy) reverse-proxy --from http://$$(./scripts/lan-ip.py):3939 \
			       --to 127.0.0.1:3901 \
...
host:
	$(wrangler) dev --ip=$$(./scripts/lan-ip.py) --port=3939
